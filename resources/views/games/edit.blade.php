        ...
        <div class="max-w-3xl w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gray-800/80 backdrop-blur-md p-8 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-gray-700 relative overflow-hidden">
                ...
                <div class="flex items-center space-x-4 mb-8">
                    ...
                    <h2 class="text-3xl font-black text-white uppercase tracking-wide">Edit: <span class="text-purple-400">{{ $game->title }}</span></h2>
                </div>

                <form action="{{ route('games.update', $game) }}" method="POST" enctype="multipart/form-data" class="space-y-6 relative z-10">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="number" name="release_year" value="{{ old('release_year', $game->release_year) }}" required
                            class="block w-full bg-gray-900 border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-shadow">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                            class="block w-full bg-gray-900 border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-shadow resize-none">{{ old('description', $game->description) }}</textarea>
                    </div>
                        ...
                        <div class="flex items-center space-x-6">
                            @if($game->image)
                                <div class="shrink-0 relative group">
                                    <img src="{{ asset('storage/' . $game->image) }}" alt="Current Image" class="w-24 h-32 object-cover rounded-lg border border-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.3)]">
                                </div>
                            @endif
                            <div class="flex-grow">
                                <p class="text-sm text-gray-500 mb-2">Upload a new image to overwrite the current cover.</p>
                                <input type="file" name="image" accept="image/*"
                                    class="block w-full text-sm text-gray-400
                                    file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0
                                    file:text-sm file:font-bold file:bg-gray-700 file:text-white
                                    hover:file:bg-purple-600 hover:file:cursor-pointer transition-all">
                            </div>
                        </div>
                    </div>
